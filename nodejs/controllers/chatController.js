...
exports.createChatSession = async (req, res) => {
    ...
    try {
        const newChatSession = new ChatSession({ name:encrypt(name) ,user: userId,image:encrypt(image) });
        ...
        user.chatSessions.push({name:encrypt(name),image:encrypt(image),sessionId:chatSession.id});
        await user.save();
...
exports.addPrompt = async (req, res) => {
    ...
    try {
        ...
        if (existingPrompt) {
            ...
            chatSession.prompts.push({ promptId, prompt:encrypt(prompt), responses: [encrypt(responsesOut[0])] });
        }
...
exports.getChatSessionData = async (req, res) => {
    try {
        ...
        chatSession.prompts.forEach(prompt => {
            ...
            prompt.responses = prompt.responses.map(response => decrypt(response)); // Decrypt each response
        });
