...
# Initialize Pinecone client using new Pinecone class
pc = Pinecone(api_key=os.environ["PINECONE_API_KEY"])

# Check if your index exists, and create it if it doesn't
index_name = "ava"  # This is your Pinecone index name
if index_name not in pc.list_indexes().names():
    pc.create_index(
        name=index_name, 
        dimension=1536,  # Ensure this matches your embedding model's output dimension
        metric='cosine',  # You can also use 'euclidean' based on your use case
        spec=ServerlessSpec(
            cloud='aws',
            region=os.environ["PINECONE_ENVIRONMENT"]  # Your Pinecone environment/region
        )
    )
...
# Define a retriever class using Pinecone
class PineconeRetriever:
    def __init__(self):
        self.index = index

    def embed(self, documentRef):
        # Check if collection exists and insert documents with embeddings
        for i, doc in enumerate(documentRef["documents"]):
            ...
            # Upsert embeddings and documents into Pinecone
            self.index.upsert(vectors=[(str(i), embedding)], namespace=documentRef["id"])

    def retrieve(self, query, top_k=3):
        query_embedding = embeddings.embed_query(query)
        # Query Pinecone index
        response = self.index.query(queries=[query_embedding], top_k=top_k, namespace="collection_1")
        # Extract relevant documents based on the result
        if response and 'matches' in response['results'][0]:
            return [match['id'] for match in response['results'][0]['matches']]
        else:
...
retriever.embed(docRef)  # Embed documents into Pinecone
